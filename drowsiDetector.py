...
def play_sound(path):
    global i, currentTime
    tmp = time.time()
    if tmp - currentTime > 7:
...
def play_warning_sound_async():
    executor.submit(play_sound, wav_path)
...
class drowsiDetector():
    ...
    def getState(self, image):
        ...
        if outputs.multi_face_landmarks:
            ...
            ratio_left = round(
                self.get_aspect_ratio(image, outputs, self.LEFT_EYE_TOP_BOTTOM, self.LEFT_EYE_LEFT_RIGHT), 2)
            ratio_right = round(
                self.get_aspect_ratio(image, outputs, self.RIGHT_EYE_TOP_BOTTOM, self.RIGHT_EYE_LEFT_RIGHT), 2)
            ...
            if self.frame_count > 30:
                self.message = 'SLEEP'
                play_warning_sound_async()

        else:
            self.message = 'FOCUS'

        cv2.putText(image, f'STATE: {self.message}', (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
                    1, (0, 0, 255) if self.message == 'SLEEP' or self.message == 'TIRED' else (0, 255, 0), 2,
                    cv2.LINE_AA)
