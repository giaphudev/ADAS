...
class objDetector(YOLO):
    ...
    def collisionLogic(self, time_collision):
        if time_collision is not None:
            if 0.8 < time_collision <= 2.5:
                color = (255, 255, 0)
                collision_state = 'Attention: Forward Obstruction' # chú ý có vật cản phía trước
            elif 0.5 < time_collision <= 0.8:
                color = (0, 255, 255)
                collision_state = 'Attention: Imminent Collision' # chú ý sắp xãy ra va chạm
            elif time_collision <= 0.5:
                color = (0, 0, 255)
                collision_state = "Warning: Approching Too Fast" # cảnh báo tiếp cận quá nhanh
            else:
                color = (0, 255, 0)
                collision_state = "Message: Safe Distance" #thông báo khoảng cách an toàn
        else:
            color = (0, 255, 0)
    ...
    def getClosestObject(self) -> list:
        self.closestObjectInfo = [None, None, "No Forward Object", None]   #KHÔNG CÓ ĐỐI TƯỢNG PHÍA TRƯỚC
        if len(self.currentObjInfo) !=0:
            ...
            if closest_object[3]:
                ...
                if closest_object[2] < 1:
                    # time_collision = 0.2
                    collision_state = 'Warning: Distance Too Close' #canh bao khoang cach qua gan
                    color = (0, 0, 255)
